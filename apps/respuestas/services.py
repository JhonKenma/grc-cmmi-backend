...
class CalculoNivelService:
    ...
    def calcular_gap_asignacion(asignacion):
        ...
        # ⭐ VALIDAR: Estado debe permitir calcular GAP
        # completado          → usuario terminó, auditor aún no revisó
        # pendiente_auditoria → flujo anterior (compatibilidad)
        # auditado            → auditor ya cerró revisión
        if asignacion.estado not in ['completado', 'pendiente_auditoria', 'auditado']:
            raise ValueError(
                f'La asignación no está disponible para calcular GAP. '
                f'Estado actual: {asignacion.estado}'
            )
        ...
        respuestas_si_cumple      = respuestas.filter(calificacion_auditor='SI_CUMPLE').count()
        respuestas_cumple_parcial = respuestas.filter(calificacion_auditor='CUMPLE_PARCIAL').count()
        respuestas_no_cumple      = respuestas.filter(calificacion_auditor='NO_CUMPLE').count()
        ...
        # Calcular GAP
        gap = nivel_deseado - Decimal(str(nivel_actual))
        ...
        print(
            f'✅ GAP {accion}: Deseado={nivel_deseado}, '
            f'Actual={nivel_actual:.2f}, GAP={gap:.2f} ({clasificacion_gap})'
        )
    ...
    def recalcular_evaluacion(evaluacion_empresa):
        ...
        # ⭐ Incluir todos los estados post-envío del usuario
